...
def process_NORMAL_images():
    _raw_folder = f'{raw_folder}/NORMAL'
    _destiny_folder = f'{destiny_folder}/NORMAL'
    _, _, images = next(walk(_raw_folder))
    for image in images:
...
def process_COVID_images():
    _raw_folder = f'{raw_folder}/COVID'
    _destiny_folder = f'{destiny_folder}/COVID'
    _, _, images = next(walk(_raw_folder))
    for image in images:
        image_path = f"{_raw_folder}/{image}"
        destiny_path = f"{_destiny_folder}/{image}"
